...
# Every score ARC disclosed that this formula does not reproduce, with the
# reason. Keeping them here rather than fitting the formula around them is the
# point: each one is a known limit of the reconstruction, not a tuning knob.
UNEXPLAINED = {
    ("sweep1", "sk48"): "death boundary — see the note above",
    ("sweep1-again", "sk48"): "death boundary — see the note above",
    ("codex-hard5", "g50t"): "scorecard records two runs under one guid; the "
    "record cannot see the boundary ARC scored across",
    ("codex-v2", "sp80"): "same as g50t: one guid, a GAME_OVER at 275 actions "
    "and a WIN at 564, scored as the second run alone",
    ("codex-v2", "sk48"): "unfinished game — ARC counts one level completed "
    "where the record reached eight, the class documented above",
}
