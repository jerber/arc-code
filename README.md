...
- **A prompt** ([`PROMPT.md`](PROMPT.md)) that never names the environment —
  ...
  left holding no game-specific fact at all.
...
runs it, writes down what it learned, and keeps going — so by the end of a game
...
The prompt says each batch of actions takes a `--plan "..."` which "becomes
your briefing history". **It never says what to put in one.**

Claude worked out on its own that the field is a channel to its future self,
and wrote derived state into it:

> `L8 systematic probe: 4 green buttons pressed 3x each so every track can be
> derived exactly from colour histories. 69 tiles, 3 yellows at lattice
> (5,1),(5,4),(5,7); brackets at (7,15),(9,15),(11,15) with lattice
> i=(x-3)/3, j=(y-6)/3.`

Codex, on the identical prompt, wrote intentions:

> `Probe green for final bridge.`

Claude's plans averaged 307 characters; Codex's 133. Codex consults its log
far *more* often — 26% of its commands touch `logs.txt` against Claude's 4% —
but gets less back, because it stored intentions rather than facts. So it
